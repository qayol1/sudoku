...
def find_repetition(row, column, number):
    row_double = 0
    column_double = 0
    for i in range(9):
    ...
    for i in range(9):
        if template[i][column][0] == number:
            row_double +=1
    if row_double > 1:
        return ("This number already exists in this column!")
    if column_double > 1:
        return ("This number already exists in this row!")
    return("")
...
def fill_cell(line):
    ...
    if len(template[row][column]) == 1:
        ...
        show()
        print(find_repetition(row, column, number))
    else:
